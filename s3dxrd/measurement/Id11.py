...
def peaks_to_vectors(flt_paths,
    ...
    for ii in range(len(rm.grain_contact_mask)):
        for jj in range(len(rm.grain_contact_mask[ii])):
            pass
    ...
    for i, (zpos, peaks) in enumerate(zip(rm.zpos, rm.peak_stack)):
        ...
        for grain_indx in np.unique(labeled_volume[:, :, i]):
            ...
            entryz, exitz, nhatz, Lz, nsegsz, bad_lines = top.get_integral_paths(angles, dty, zpos, sample_polygon,
                                                                                 nprocs=nprocs, show_geom=False)
